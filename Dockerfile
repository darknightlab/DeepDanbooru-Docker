...
RUN apt update && apt install git -y && git clone https://github.com/darknightlab/DeepDanbooru /root/deepdanbooru/DeepDanbooru && cd /root/deepdanbooru/DeepDanbooru && pip install --no-cache-dir -r requirements.txt && chmod +x setup.py && pip install . && apt remove git -y && apt autoremove -y && rm -rf /root/deepdanbooru/DeepDanbooru
